#!/usr/bin/env python3
"""通过 ADB 运行 Voice Satellite 0.1.2+ 的完整 Probe（三路麦克风 + 扬声器播放）并导出报告。

在 r1_probe_ui.py 基础上泛化：额外要求报告包含 [PLAYBACK] 段，等待预算覆盖
最坏情况（3x8s 麦克风超时 + 8s 播放超时），并对本固件偶发的 "error: closed"
ADB 断连做有限重试。
...
SOURCES = ("MIC", "VOICE_RECOGNITION", "VOICE_COMMUNICATION", "PLAYBACK")
# 报告等待预算：正常约 15 秒；最坏 3 路麦克风各 8 秒硬超时 + 播放 8 秒 + 界面刷新余量。
REPORT_BUDGET_SECONDS = 70
...
def read_report(root):
    ...
    if not all("[" + s + "]" in report for s in SOURCES):
        return None  # 播放段出现前报告也可能完整展示三路结果；必须等到 [PLAYBACK]。
    return report


class Device:
    def __init__(self, adb, serial, folder):
        self.adb_bin = adb
        self.serial = serial
        self.folder = folder
        self.last_xml = None
    ...
    def adb(self, args, timeout=20):
        # 本固件的 adbd 偶发 "error: closed" 断连；有限重试并显式重连，不无限循环。
        last = None
        for attempt in range(3):
            try:
                result = subprocess.run([self.adb_bin, "-s", self.serial] + args,
            ...
            last = text
            if "error: closed" in text or "not found" in text or "offline" in text:
                time.sleep(2)
                subprocess.run([self.adb_bin, "connect", self.serial],
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=15)
                continue
            raise ProbeError("ADB 命令失败：\n" + text)
        raise ProbeError("ADB 命令重试后仍失败：\n" + (last or ""))

    def shell(self, command, timeout=20):
        # 老 adbd 可能不回传远端退出码，用标记验证。保留设备端 sh -c 的引号。
        # 注意：本固件 adbd 会拒绝顶层 pm 命令，sh -c 包装同时也是必需而非可选。
        marker = "__R1_RC_" + uuid.uuid4().hex + "__"
        # R1 的精简 shell 没有 printf；用内建 echo 回传退出码。
...
def main():
    ...
    parser.add_argument("--adb", default=os.environ.get("ADB") or shutil.which("adb") or "adb",
                        help="adb 可执行文件路径（默认读 ADB 环境变量或 PATH）")
    ...
    adb_path = args.adb if Path(args.adb).is_file() else shutil.which(args.adb)
    if adb_path is None:
        print("找不到 adb：" + args.adb + "（可用 --adb 或 ADB 环境变量指定）", file=sys.stderr)
        return 1
    stamp = time.strftime("%Y%m%d-%H%M%S") + "-" + uuid.uuid4().hex[:6]
    folder = Path(args.out).expanduser().resolve() / ("playback-" + stamp)
    folder.mkdir(parents=True, exist_ok=False)
    device = Device(adb_path, args.serial, folder)
    tapped = False
    try:
        ...
        if args.action == "run":
            x, y = button_center(root)
            print("\n即将运行三路麦克风 Probe + 扬声器播放诊断。", flush=True)
            print("全程保持安静。" if args.label == "quiet" else
                  "保持相同距离、音量，开始后连续重复同一句话，直到打印完整报告；"
                  "播放阶段会听到约 1.5 秒 440 Hz 测试音。", flush=True)
            for value in (3, 2, 1):
            ...
            while True:
                ...
                progress = next((t for t in texts if t.startswith(("Testing ", "Playing "))), "尚未读到完成报告")
        ...
        path = folder / ("r1-probe-playback-" + args.label + ".txt")
        path.write_text(report + "\n", encoding="utf-8")
        print("\n" + report)
        print("报告已保存：", path)
        warnings = [w for w in ("ERROR:", "init=false", "recording=false", "timeout=true",
                                "played=false", "simultaneous=false", "TRACK ERROR") if w in report]
        if warnings:
            print("注意：已成功导出报告，但存在异常标记 %s；这不等于硬件验证通过。" % warnings)
            return 2
        return 0
