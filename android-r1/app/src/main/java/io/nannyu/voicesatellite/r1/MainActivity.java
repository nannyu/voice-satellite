...
public final class MainActivity extends Activity {
    private TextView status; private AudioRecorder recorder; private AudioProbe probe; private long frames; private Button probeButton;
    @Override public void onCreate(Bundle b){super.onCreate(b); LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);status=new TextView(this);status.setText("Voice Satellite R1\nAPI 22 / armeabi-v7a diagnostics");
        probeButton=new Button(this);probeButton.setText("Run automatic R1 Audio Probe");probeButton.setOnClickListener(v->runProbe());root.addView(probeButton);
        ...
        Button capture=new Button(this);capture.setText("Start / Stop microphone");capture.setOnClickListener(v->toggleCapture());root.addView(capture);
        ScrollView scroll=new ScrollView(this);scroll.addView(status);root.addView(scroll,new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,0,1));setContentView(root);}
    private void runProbe(){if(probe!=null){probe.cancel();return;}if(recorder!=null&&recorder.isRunning()){recorder.close();recorder=null;}probe=new AudioProbe();probeButton.setText("Cancel Audio Probe");status.setText("Starting probe...");probe.start(new AudioProbe.Listener(){public void onProgress(String text){runOnUiThread(()->status.setText(text));}public void onComplete(String report){runOnUiThread(()->{status.setText(report);probe=null;probeButton.setText("Run automatic R1 Audio Probe");});}});}
